    ...
    foreach ($sele as $ect) {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($product_image_tmp_name, $product_image_folder)) {
                $addAuction = $conn->query("INSERT INTO `products`(`product_name`, `price`, `description`, `category`, `product_image`) VALUES ('$productName', '$price', '$details', '" . $ect['category_id'] . "', '$product_image')");
